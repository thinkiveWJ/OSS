	...
	$.jqPagination = function (el, options) {
		...
		base.setMaxPage = function (max_page, prevent_paged) {
			...
			if (max_page < base.options.current_page) {
			    ...
			    //return false;
			    base.options.current_page = max_page;
			    base.$input.data('current-page', max_page);
			}
		...
		base.callMethod = function (method, key, value,isc) {
			...
			switch (method.toLowerCase()) {
				...
				case 'option':
					...
					if(options.max_page !== undefined){
					    ...
					    //result = base.setMaxPage(options.max_page, true);
					}
	...
	$.fn.jqPagination = function () {
		...
		if (typeof args[0] === 'string') {
			...
			if (args[2] === undefined) {
				...
				$self.each(function(){
					result = $(this).data('jqPagination').callMethod(args[0], args[1], args[2],args[3]);
				});
