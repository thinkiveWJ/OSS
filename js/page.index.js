		...
		$("<div><br/><span>将会销毁当前用户会话,并会放弃所有未保存的操作。</span></div>").dialog({
			...
			buttons : {
				"确认" : function() {
					...
					window.top.location.href=window.top.location.href.substring(0,window.top.location.href.indexOf("platform.do"));
//					$.ajax({
...
//                    });   
	...
	$("#contentFrame").bind("load", function() {
		...
		if (selectedTopMenu) {
			...
			$('<li>' + data.menuName + '</li>').appendTo($ul);
			...
			if (selectedLeftMenu) {
				...
				if ($parent.is(".nohover")) {
					$('<li class="aw-right">&gt;</li>').appendTo($ul);
					$('<li>' + $parent.data().menuName + '</li>').appendTo($ul);
				}
				$('<li class="aw-right">&gt;</li>').appendTo($ul);
				$('<li class="active">' + $tslitem.data().menuName + '</li>').appendTo($ul);
			}
		} else {
	...
	window.backHome = function() {
		...
		window.frames["contentFrame"].location.replace("pages/dashboard.html");
		swithMode("noleft");
	};
	//定义弹出窗体（可以覆盖到最大）
	window.frameDialog = function(url, title, opts) {
		...
		var tf=$this.find("iframe")[0].contentWindow,data=opts.data;
		tf.onload=function(){
			if(tf.setValue){
				tf.setValue(data);
			}
	...
	window.frameUnMask = function() {
        ...
        window.fullShipFrame=function(url,title,buttons){
            var $el=$("<div class='full-ship-panel'>"),
                    ...
                    $frameEl=$('<div class="full-ship-panel-content"><iframe frameborder="0"  marginheight="0" marginwidth="0" class="drop-frame"></iframe></div>').appendTo($el).find("iframe");
	...
	var switchUser = function(roleId) {
		var url = "files/json/getUserInfo.json"+(roleId?"&roleId="+roleId:"");
		$("body").mask("正在读取用户信息...");
...
//			$icon.attr("src", icon).next().text("钟志军");
			$icon.attr("src", icon).next().text(sessionStorage.getItem("loginUserName"));
			$icon.attr("src", icon);
