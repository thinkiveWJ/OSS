...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            dragDropStr: "<span><b>可拖拽文件到此处</b></span>",
            abortStr: "取消",
            cancelStr: "不保留",
            deletelStr: "删除",
            doneStr: "确认",
        ...
        if (this.width() !== 0) {
            s. statusBarWidth=   s.dragdropWidth = this.width();
        }
        this.fileCounter = 1;
        ...
        s.ISDISABLED=obj.data("disabled");
        s.fileList=$('<div id="'+($(this).attr("id")|| $(this).attr("name"))+'_list" class="file-area'+(s.ISDISABLED?" readonly":"")+'" style="width:'+s.dragdropWidth+'px;"></div>');
        if(appendto){
        	s.fileList.appendTo(appendto);
        }else{
        	$(this).before(s.fileList);
        }
        s.fileList.click(function (ev) {
        	var query = $(ev.target).closest(".remove");
            if(query.length!==0){
                //console.log(ev);
                var tempel = query.closest("span"),
                tid = tempel.find("a:eq(0)").data("fid");
                if (s.removeUrl) {
                    $.get(s.removeUrl+(s.removeUrl.indexOf("?")===-1?"?id=":"&id=")+tid);
                }
                tempel.hide(800).remove();
                if(obj.valid){
                    try { obj.valid();}catch(ex){}
                }
            }
        });

        this.fillFileItems = function (data, stepValid) {
            if (data) {
                var $efileList = s.fileList;
                for (var i in data) {
                    var item = data[i],
                        filename = item.fileName,
                        path = item.path,
                        id = item.id,href=path;
                    if(s.dowloadUrl){
                    	href= s.dowloadUrl.replace(/{id}/g,id);
                    }
                    var $tel = $("<span><a class='file-name' data-fid='" + id + "' href='"+href+"'><i class='fa fa-file-text-o' style='margin-right:3px;font-size:14px;'></i>" + filename + "</a><a title='移除' class='remove'><i  class='fa fa-times' ></i></a></span>");
                    $tel.appendTo($efileList);
                }
                if (obj.valid && !stepValid) {
                    try { obj.valid();}catch(ex){}
                }
            }
        };
        this.clearFileItems = function () {
            var $efileList = s.fileList;
            $efileList.find("span").remove();
        };
        this.data({
            "fileItems": function (org) {
                if (org != undefined) {
                    obj.clearFileItems();
                    obj.fillFileItems(org,true);
                    return obj;
                } else {
                    var $efileList = s.fileList,result=[];
                    $efileList.find("span").each(function (index, item) {
                        var $item = $(item).find("a:eq(0)"),
                            id = $item.data("fid"),
                            path = $item.attr("href"),
                            fileName = $item.text();
                        result.push({"id":id,"path":path,"fileName":fileName});
                    });
        ...
        var uploadLabel = $('<div>' + ($(this).data("button-text") || $(this).html()) + '</div>');
        $(uploadLabel).addClass(s.uploadButtonClass);
        //======================CHANGE (DISABLED)
        if(s.ISDISABLED){
       	 $(uploadLabel).addClass("readonly");
        }
        //============================
        ...
        (function checkAjaxFormLoaded() {
            if($.fn.ajaxForm) {
                ...
                if(s.dragDrop) {
                    ...
                    $(obj).before($("<div style='height:30px;display:block;'>").append(uploadLabel));
                    //========================================CHANGE=================================
                   // console.log(obj);
                    uploadLabel.css({"margin-left":0,"margin-top":0});
                    //=========================================CHAGE END=============================
                }
        ...
        function setDragDropHandlers(obj, s, ddObj) {
            ...
            ddObj.on('dragenter', function (e) {
                e.stopPropagation();
                e.preventDefault();
                $(this).css({ 'color': '#2f8ab9', "box-shadow": " 0 0 6px #0C376D inset" });
            ...
            ddObj.on('drop', function (e) {
                $(this).css({ 'color': '#c9c9c9', "box-shadow": "none" });
                e.preventDefault();
                obj.errorLog.html("");
            ...
            $(document).on('dragover', function (e) {
                e.stopPropagation();
                e.preventDefault();
                ddObj.css({ 'color': '#c9c9c9', "box-shadow": "none" });
            });
        ...
        function createCutomInputFile(obj, group, s, uploadLabel) {
            ...
            fileInput.change(function () {
                ...
                if(this.files) //support reading files
                {
                    ...
                    //====================================CHANGE==================================
                    //console.log(s.dragDrop+"ddddddd");
                    if (!s.dragDrop) {
                        uploadLabel.hide();
                    }
                    //====================================CHANGE END================================
            ...
            var options = {
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.returnType == "json" && $.type(data) == "object" && data.hasOwnProperty(s.customErrorKeyStr)) {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span style='color:red;'>发成错误: " + msg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
                ...
                error: function (xhr, status, errMsg) {
                    ...
                    if(xhr.statusText == "abort") //we aborted it
                    {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            pd.statusbar.append("<span style='color:red;'>发生错误: " + errMsg + "</span>");
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
