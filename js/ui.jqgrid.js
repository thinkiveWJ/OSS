		...
		var ts= this, grid={
			...
			dragEnd: function() {
				...
				if(this.resizing) {
					...
					this.headers[idx].el.style.width = (nw<0?0:nw) + "px";
					this.cols[idx].style.width = (nw<0?0:nw)+"px";
					if(this.footers.length>0) {this.footers[idx].style.width = (nw<0?0:nw)+"px";}
					if(p.forceFit===true){
						nw = this.headers[idx+p.nv].newWidth || this.headers[idx+p.nv].width;
						this.headers[idx+p.nv].width = nw;
						this.headers[idx+p.nv].el.style.width = (nw<0?0:nw) + "px";
						this.cols[idx+p.nv].style.width = (nw<0?0:nw)+"px";
						if(this.footers.length>0) {this.footers[idx+p.nv].style.width =(nw<0?0:nw)+"px";}
					...
					$.ajax($.extend({
						...
						success:function(data,st, xhr) {
							if(data.rspCode&&data.rspCode<=0){
								...
								data={records:0,rows:[],total:0};
							}
							if($("table.integral-system-details")&&data['data']){
								var $td=$("table.integral-system-details tbody tr:eq(0) td")
								$td.eq(0).text("姓名："+data['data']['custName']);
								$td.eq(1).text("当前积分："+data['data']['remainPoint']);
								$td.eq(2).text("总积分："+data['data']['totalPoint']);
								$td.eq(3).text("已使用积分："+data['data']['usedPoint']);
							}
//							if(sessionStorage.getItem("goodsListStep1")=="true"){//wj2016-05-24
//								var goodsListStep1Result=data.rows;
//								$("#chooseSaveStep2").data("goodsListStep1Result",goodsListStep1Result);
//							}
//							if(sessionStorage.getItem("goodsListStep3")=="true"){//wj2016-05-24
//								var goodsListStep1Result=data.rows;
//								$("#chooseSaveStep3").data("goodsListStep3Result",goodsListStep1Result);
//							}
							if(data.rows.length==1&&data.rows[0]==null){
								data={records:0,rows:[],total:0};
							}
		...
		return this.each(function(){
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$th.before($colHeader); // CustomerTakesServiceImpl new column header before the current
			...
			if (o.url === 'clientArray') {
				...
				$.ajax($.extend({
					...
					complete: function(res,stat){
						if(res.rows.length==1&&res.rows[0]==null){
							return;
						}
					...
					if(cv[0] === true) {
						...
						if ($t.p.cellsubmit === 'remote') {
							if ($t.p.cellurl) {
								...
								var data=$.isFunction($t.p.serializeCellData) ? $t.p.serializeCellData.call($t, postdata) : postdata;
								/***********wj2016-06-23 start   修改商品的最大值最小值新增step1*********/
								if(window.top["goodsInputDataJson"]){
									var goodsInputDataJson=window.top.goodsInputDataJson;
									var findItemByPromotionIDUrl=goodsInputDataJson.findItemByPromotionIDUrl;
									var isGiveGoods=goodsInputDataJson.isGiveGoods
									$.ajax({
										url:findItemByPromotionIDUrl,
										data:JSON.stringify({promotionID:goodsInputDataJson.promotionID,isGiveGoods:isGiveGoods}),
										type:"post",
										contentType: 'application/json',
										dataType:"JSON",
										success:function(result){
											...
											var index=data["id"]-1;
											var promotionID=dataList[index]["promotionID"];
											data.promotionID=promotionID?promotionID:"";
											var skuId=dataList[index]["skuID"];
											data.skuId=skuId?skuId:"";
											var bigTypeCode=dataList[index]["bigTypeCode"];
											data.bigTypeCode=bigTypeCode?bigTypeCode:"";
											var midTypeCode=dataList[index]["middleTypeCode"];
											data.midTypeCode=midTypeCode?midTypeCode:"";
											var smaTypeCode=dataList[index]["smallTypeCode"];
											data.smaTypeCode=smaTypeCode?smaTypeCode:"";
											data.isGiveGoods=isGiveGoods;
											data=JSON.stringify(data);
											$.ajax( $.extend( {
												url: $t.p.cellurl,
												data :data,
												type: "POST",
												contentType: 'application/json',//wj2016-06-08
			...
			"drag" : function (opts) {
				return $.extend({
					start : function (ev, ui) {
						...
						// hack
						// drag and drop does not CustomerTakesServiceImpl tr in table, when the table has no rows
						// we try to CustomerTakesServiceImpl new empty row on the target(s)
...
(function ($) {
    ...
    $.jgrid.extend({
        ...
        autoFillWidth: function () {
            return this.each(function (index, item) {
                var $self = $(this), grid = this.grid, aw = grid.width, colModel, needResAry = [], ofsWidth = aw, resiW = 5, scrollW = 24;
                colModel = $self.jqGrid("getGridParam", "colModel");
                for (var i = 0, nCol = colModel.length; i < nCol; i++) {
                    if (colModel[i].hidden) { continue; }
                    if (colModel[i].autoWidth) {
                        needResAry.push({ index: i });
                    } else {
                        ofsWidth -= (colModel[i].width + resiW);
                    }
                }
                ofsWidth -= scrollW;
                if (needResAry.length === 0) {
                    return;
                }
                var pjW = ofsWidth / needResAry.length - 5;
                for (var i = 0, nCol = needResAry.length; i < nCol; i++) {
                    grid.resizing = { idx: needResAry[i].index };
                    grid.headers[needResAry[i].index].newWidth = pjW;
                    grid.dragEnd();
                }
            });
        }
