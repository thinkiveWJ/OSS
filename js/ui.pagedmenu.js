...
//            $.ajax({"url":this.options.sourceUrl,data: JSON.stringify({userId:userId}),
//            	type:"post",contentType: 'application/json',dataType:"json",success:function(result){
...
//		    		}
//            		//清除缓存
//            		vthis._clearCacheFunc(result.rows);
//            	}
//            });
            /******end wj2016.05.20******/
        ...
        },_clearCacheFunc:function(result){
        	var vthis = this;
        	if(result.length>0){
        		for(var i in result){
        			if(result[i].menus&&result[i].menus.length>0){
        				result[i].menus["url"]=result[i].menus["url"]+"?v="+(new Date()).getTime()+"m"+Math.random();
        				vthis._clearCacheFunc(result[i].menus);
        			}
        		}
        	}
        	vthis._clearItems();
            vthis._addItems(result);
            vthis._initContentConfig();
            vthis._refresh();
        },
        _clearItems: function () {
            ...
            for (var i = 0, len = items.length; i < len; i++) {
                item = items[i];
                elAry.push($("<div class='ui-pagedmenu-menu-item'>").text(item.menuName).data(item).addClass(item.cls));
            }
            $cel.append(elAry);
